...
return AbstractModel.extend({
        ...
        if (event.allDay) {
            if (!this.mapping.all_day) {
                if (event.r_start) {
                    ...
                    start.hours(7).add(-this.get_user_offset(start), 'minutes');
                    end.hours(19).add(-this.get_user_offset(end), 'minutes');
                }
            }
        } else {
            start.add(-this.get_user_offset(start), 'minutes');
            end.add(-this.get_user_offset(end), 'minutes');
        }
    ...
    createRecord: function (event) {
        ...
        if (data['dob']) {
            data['dob'] = dateToServer(data['dob']);
        }
        for (var i in data) {
...
//                context: event.data.options.context,
    ...
    load: function (params) {
        ...
        if (!this.preload_def) {
            ...
            $.when(
                ...
                this._rpc({model: 'calender.config', method: 'find_model_options'}),
                )
            .then(function (write, create, doctor_ids, other_options) {
                self.write_right = write;
                self.create_right = create;
                self.doctor_ids = doctor_ids;
                self.user_offset = other_options[0];
                self.show_cancelled = other_options[1];
    ...
    /*setDate: function (start) {
        ...
        this.data.start_date = this.data.end_date = start;
        this.data.start_date.utc().add(this.get_user_offset(this.data.start_date), 'minutes');
...
//        return new Date(dateString).toUTCString();
    ...
    _loadCalendar: function () {
		...
		if (self.data.state_domain) {
		    ...
		    let st_vals = ["draft", "confirmed", "checkin", "ready", "done", "visit_closed"];
		    if (self.show_cancelled == 'True') {
                st_vals.push('missed');
                st_vals.push('cancel');
            }
			domain_val.push(['state', 'in', st_vals]);
     ...
     */
    get_user_offset: function (date_val) {
        if (this.user_offset)
            return this.user_offset;
        return this.getSession().getTZOffset(date_val);
    },
    _recordToCalendarEvent: function (evt) {
        ...
        if (!all_day) {
            date_start.add(this.get_user_offset(date_start), 'minutes');
            date_stop.add(this.get_user_offset(date_stop), 'minutes');
        }
